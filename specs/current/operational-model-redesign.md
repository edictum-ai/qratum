...
- Output priority:
  ...
  5. Review.
  6. Corpus.
- Primary surface: CLI + vault; app earned later.
- CLI role: hook install/status, archive, backfill, doctor, backup, import,
  failed/retry, logs, automation, and debug.
...
```txt
sqlite
  default local projection for search and local library views when search is
  unlocked
...
qrt vault doctor
qrt vault backfill
qrt vault archive <path>
qrt vault backup [--verify] <dest>
qrt setup
qrt open
qrt status
qrt failed
...
- Shows workspace and vault status.
- Shows hook/backfill/backup health.
- Surfaces recent failures that need attention.
...
Diagnostics stay CLI-first until an app earns its place. Public CLI exposes
status, vault health, failed, retry, and logs. Detailed job/store commands
stay debug/internal.
...
- No raw content leaves the machine by default.
