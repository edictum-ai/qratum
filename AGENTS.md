...
authoritative for onboarding and the public `qrt` command contract (see §Source
Of Truth above). Where a generic phrasing here conflicts with a repo rule, the
repo rule wins.
...
- Build the least machinery the contract asks for. No unrequested parsers,
  validators, or abstractions (mirrors §Still not built). If the simple approach
  feels insufficient, stop and ask — don't build.
...
### Acceptance suite (deferred)

`test/acceptance/` and its `phases.json` activation convention are documented
here for when the first frozen suite lands. Wiring `acceptance-dir` into the Go
test runner (e.g. a `make` target) is deferred to that first suite; no acceptance
content is created as part of onboarding.
